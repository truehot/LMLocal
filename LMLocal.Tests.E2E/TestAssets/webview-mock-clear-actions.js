// Mock for Clear Chat action tests.
// Simulates streaming like webview-mock-streaming.js but records the
// ResetHistoryWithActionAsync action / SummarizeAndCompactAsync model and
// returns a non-empty last session after a clear so the frontend re-renders
// carried-over history.
const _listeners = [];
const _timers = [];
window._listeners = _listeners;
window._mock_timers = _timers;
window.__resetAction = null;
window.__summarizeModel = null;
...
const __mockBridge = {
    ...
    ResetHistoryWithActionAsync: async (action) => {
        window.__resetAction = action;
        return true;
    },
    SummarizeAndCompactAsync: async (modelId) => {
        window.__summarizeModel = modelId;
        return true;
    },
    ...
    if (typeof window.lmInit === 'function') {
        ...
        window.__providersOverride = {
            GetProvidersAsync: async () => JSON.stringify({
                ...
                data: {
                    providers: [],
                    providerTypes: [{ key: 'openai', displayName: 'OpenAI' }, { key: 'ollama', displayName: 'Ollama' }]
                }
            }),
        ...
        window.__toolsOverride = {
            GetToolsAsync: async () => JSON.stringify({
                success: true,
                data: {
                    tools: [{ id: 'tool-1', name: 'read_file', description: 'Read file contents', enabled: true }, { id: 'tool-2', name: 'write_file', description: 'Write file contents', enabled: false }, { id: 'tool-3', name: 'search_files', description: 'Search for files', enabled: true }]
                }
            }),
        ...
        window.__chatSessionOverride = {
            GetLastChatSessionAsync: async () => JSON.stringify({
                hasSession: true,
                messages: [
                    { role: "user", content: "moved prompt" },
                    { role: "assistant", content: "moved response" }
                ]
            }),
            GetCurrentChatHistoryAsync: async () => JSON.stringify({
                hasSession: true,
                messages: [
                    { role: "user", content: "moved prompt" },
                    { role: "assistant", content: "moved response" }
            ...
            GetChatSessionByIdAsync: async (sessionId) => JSON.stringify({
                hasSession: true,
                messages: [
                    { role: "user", content: "moved prompt" },
                    { role: "assistant", content: "moved response" }
