    ...
    internal static class SseStreamParser
    {
        ...
        public static SseMessage TryParseSseLine(string line)
        {
            ...
            if (line.StartsWith(CommentPrefix))
                return new SseMessage { Type = SseMessageType.Comment, RawData = line };

            if (line.StartsWith(EventPrefix))
            {
                var eventType = line.Substring(EventPrefix.Length).Trim();
                return new SseMessage { Type = SseMessageType.Event, EventType = eventType };
            }
            ...
            if (line.StartsWith(DataPrefix))
            {
                ...
                if (dataJson == DoneMarker)
                    return new SseMessage { Type = SseMessageType.Done, RawData = dataJson };
                return new SseMessage { Type = SseMessageType.Data, RawData = dataJson };
            }
        ...
        {
            if (!line.StartsWith(EventPrefix)) return null;
